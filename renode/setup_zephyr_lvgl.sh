...
# Usage: ./setup_zephyr_lvgl.sh [WORKDIR] [BOARD] [DEMO] [RESOLUTION]
#
# DEMO choices:
...
if [ "$BOARD" != "stm32f746g_disco" ]; then
  log "ERROR: Unsupported board '$BOARD'. This setup only supports stm32f746g_disco."
  exit 1
fi
...
log "=== Zephyr+LVGL build for $BOARD / demo=$DEMO / resolution=$RESOLUTION ($(date)) ==="
log "WORKDIR: $WORKDIR  Display: $GUI_MODE"

# Resolve the script's own directory *before* any pushd/cd below, so the
# boards/ overlay lookup is independent of the caller's CWD. BASH_SOURCE[0]
# is preferred over $0 because $0 may be a relative path (e.g. when the
# script is invoked as `./setup_zephyr_lvgl.sh` from inside WORKDIR).
...
# ── 5: Build LVGL sample ──────────────────────────────────────────────────────
log "[5/5] Building LVGL demo '$DEMO' for $BOARD at $RESOLUTION..."
...
BUILD_DIR="$WORKDIR/build_${BOARD}_${DEMO}_${RESOLUTION}"
export ZEPHYR_TOOLCHAIN_VARIANT=zephyr
export CCACHE_DIR="$WORKDIR/.ccache"   # persistent cache for incremental rebuilds

# Select matching Zephyr and Renode descriptions. The native resolution uses
# the upstream board files; 800x400 uses the external-panel overrides.
case "$RESOLUTION" in
  480x272)
    DISPLAY_OVERLAY=""
    ...
    ;;
  800x400)
    DISPLAY_OVERLAY="$SCRIPT_DIR/boards/stm32f746g_disco_800x400.overlay"
...
[ -n "$DEMO_KCONFIG" ] && BUILD_OPTIONS+=("$DEMO_KCONFIG")
[ -n "$DISPLAY_OVERLAY" ] && BUILD_OPTIONS+=("-DDTC_OVERLAY_FILE=$DISPLAY_OVERLAY")
...
# Renode script for ${BOARD} at ${RESOLUTION} — generated by setup_zephyr_lvgl.sh
...
RESC_EOF
  ...
  log "  or: ./run_renode_lvgl.sh $WORKDIR $BOARD $DEMO $RESOLUTION"
else
  log "[INFO] Renode not found — skipping .resc generation."
  log "[INFO] Run install_renode.sh, then: ./run_renode_lvgl.sh $WORKDIR $BOARD $DEMO $RESOLUTION"
fi
